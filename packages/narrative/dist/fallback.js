 ...
 */
export function generateFallbackNarrative(input) {
    const { summary } = input;
...
function buildPositionParagraph(input) {
    const { project, summary } = input;
    const sentences = [];
    sentences.push(`${project.code} ${project.name} tracks ${summary.totalAssumptions} assumptions at the date of this brief.`);
    if (summary.breachingNow > 0) {
        sentences.push(`${summary.breachingNow} ${pluralise(summary.breachingNow, 'assumption is', 'assumptions are')} currently outside tolerance and ${summary.breachingWithin30d} further ${pluralise(summary.breachingWithin30d, 'is', 'are')} projected to breach within 30 days.`);
    }
    else if (summary.breachingWithin30d > 0) {
        sentences.push(`No assumption is currently in breach, although ${summary.breachingWithin30d} ${pluralise(summary.breachingWithin30d, 'is', 'are')} projected to breach within 30 days.`);
    }
    else {
        sentences.push(`No assumption is currently in breach and none is projected to breach within 30 days.`);
    }
    sentences.push(`Overall drift score stands at ${percent(summary.overallDriftScore)} and system fragility at ${percent(summary.globalFragility)}.`);
    return sentences.join(' ');
}
function buildNearTermParagraph(input) {
    const { summary } = input;
    ...
    const cascadeFragment = topDriver !== undefined
...
function buildCallToActionParagraph(input) {
    const { summary } = input;
    const first = summary.mostAtRisk[0];
    if (first === undefined) {
    ...
    const window = first.leadTimeDays <= 30
        ? 'within ten working days'
        : first.leadTimeDays <= 90
            ? 'within twenty working days'
            : 'within thirty working days';
    const topDriver = summary.topDrivers[0];
    const driverMention = topDriver !== undefined
