...
export function buildUserPrompt(input: NarrativeInput): string {
  ...
  if (summary.mostAtRisk.length > 0) {
    ...
    for (const r of summary.mostAtRisk) {
      lines.push(
        `- ${r.code} ${r.description}: ${r.leadTimeDays} days, severity ${r.severity}, confidence ${r.confidence.toLowerCase()}`,
      );
    }
    lines.push('');
  }
  ...
  if (summary.externalSignals.length > 0) {
    ...
    for (const s of summary.externalSignals) {
      lines.push(
        `- ${s.code} ${s.description}: baseline ${s.baselineValue}, current ${s.currentValue.toFixed(2)}, drift ${s.driftPct.toFixed(1)} per cent, retrieved ${s.lastRetrievedAt.toISOString()}`,
      );
    }
    lines.push('');
  }
