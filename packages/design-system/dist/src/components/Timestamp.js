...
function formatAbsolute(target) {
    return target
        .toISOString()
        .replace('T', ' ')
        .replace(/\.\d{3}Z$/, 'Z');
}
export const Timestamp = forwardRef(function Timestamp({ at, refreshMs = 15_000, mode = 'relative', style, ...rest }, ref) {
    ...
    const display = mode === 'absolute' ? absolute : mode === 'both' ? `${relative} (${absolute})` : relative;
    return (_jsx("time", { ref: ref, dateTime: target.toISOString(), title: absolute, "aria-label": `${relative}, ${absolute}`, style: {
            fontFamily: fontFamily.mono,
            ...scaleStyle('monoSmall'),
            color: colour.ink.tertiary,
            ...style,
        }, ...rest, children: display }));
});
