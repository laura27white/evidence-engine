...
const variantStyle: Record<BadgeVariant, { background: string; border: string; color: string }> = {
  safe: {
...
const DefaultIcon: Record<BadgeVariant, typeof Info> = {
  safe: CheckCircle,
...
export const Badge = forwardRef<HTMLSpanElement, BadgeProps>(function Badge(
  { variant = 'neutral', size = 'md', icon, children, style, ...rest },
  ref,
) {
  ...
  const resolvedIcon =
    icon === null ? null : (icon ?? <DefaultIconComponent size={iconSize} aria-hidden />);
  const composed = {
    display: 'inline-flex',
    alignItems: 'center',
    gap: space.scale['2'],
  ...
  return (
    <span ref={ref} style={composed} role="status" data-variant={variant} {...rest}>
      {resolvedIcon}
      <span>{children}</span>
    </span>
  );
