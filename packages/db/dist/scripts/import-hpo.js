...
export function parseXlsx(path) {
    ...
    if (!firstSheetName)
        throw new Error(`No sheet found in ${path}`);
    const firstSheet = workbook.Sheets[firstSheetName];
    if (!firstSheet)
        throw new Error(`Sheet ${firstSheetName} missing from ${path}`);
    const rawRows = XLSX.utils.sheet_to_json(firstSheet, { defval: null });
    return rawRows.map((raw, index) => {
        const parsed = xlsxRowSchema.safeParse(raw);
        if (!parsed.success) {
            throw new Error(`Row ${index + 2} failed validation: ${parsed.error.issues
...
async function main() {
    ...
    const inserts = rows.map((row) => mapRow(row, projectId));
    ...
    console.log(`[import-hpo] ${inserts.length} assumptions upserted, ${cascadeInserts.length} cascade links upserted.`);
}
...
//# sourceMappingURL=import-hpo.js.map