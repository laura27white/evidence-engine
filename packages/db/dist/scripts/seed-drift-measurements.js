...
function synthesiseSeries(row, rng) {
    ...
    for (let i = 0; i < MEASUREMENT_COUNT; i += 1) {
        const dailyDriftSum = Array.from({ length: daysPerMonth }, () => gaussian(rng, params.mu, params.sigma)).reduce((sum, d) => sum + d, 0);
...
async function main() {
    ...
    if (reset) {
        const { error: delErr } = await client
            ...
            .delete()
            .in('assumption_id', assumptions.map((a) => a.id));
        if (delErr) {
        ...
        if ((count ?? 0) > 0) {
            console.log(`[seed-drift] ${count} measurements already present; skipping. Re-run with --reset to rebuild.`);
            return;
        }
    }
        ...
        if (stub) {
            ...
            for (let i = 0; i < MEASUREMENT_COUNT; i += 1) {
                ...
                inserts.push({
                    ...
                    notes: 'Synthetic. Gaussian drift seeded by likelihood_of_failure; see SYNTHETIC_DATA.md.',
                });
            }
        }
    }
    ...
    console.log(`[seed-drift] inserted ${inserts.length} measurements across ${assumptions.length} assumptions.`);
}
main().catch((err) => {
    console.error('[seed-drift] failed:', err instanceof Error ? err.message : err);
    process.exit(1);
});
//# sourceMappingURL=seed-drift-measurements.js.map