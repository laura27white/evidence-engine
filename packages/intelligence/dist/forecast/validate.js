...
export function prepareSeries(measurements, now) {
    if (measurements.length === 0) {
        throw new ForecastInputError('Cannot forecast from empty measurement series');
    }
    for (const m of measurements) {
        if (!Number.isFinite(m.observedValue)) {
            throw new ForecastInputError(`Non-finite observedValue for assumption ${m.assumptionId} at ${m.measuredAt.toISOString()}`);
        }
    }
...
export function rootMeanSquare(residuals) {
 ...
 */
export function hashSeries(measurements) {
    const sorted = [...measurements].sort((a, b) => a.measuredAt.getTime() - b.measuredAt.getTime() ||
