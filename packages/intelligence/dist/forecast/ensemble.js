...
export function ensembleForecast(measurements, baseline, config = {}) {
    ...
    const membersWithBreach = breachDates.filter((d) => d !== null);
    ...
    const upperBounds = members
        ...
        .filter((d) => d !== undefined);
    const breachConfidenceInterval = breachDate !== null && lowerBounds.length > 0 && upperBounds.length > 0
        ? [
 ...
 */
export function computeEnsembleAgreement(breachDates, now, horizonDays) {
    const days = breachDates.map((d) => d === null ? horizonDays : Math.max(0, (d.getTime() - now.getTime()) / 86_400_000));
