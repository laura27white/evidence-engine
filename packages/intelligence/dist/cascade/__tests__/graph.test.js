...
describe('buildGraph', () => {
    ...
    it('rejects a cycle and reports the cycle path', () => {
        const result = buildGraph([node('A'), node('B'), node('C')], [edge('A', 'B', 0.5), edge('B', 'C', 0.5), edge('C', 'A', 0.5)]);
        expect(result.ok).toBe(false);
...
describe('topologicalSort', () => {
    it('orders a chain correctly', () => {
        const result = buildGraph([node('A'), node('B'), node('C')], [edge('A', 'B', 0.5), edge('B', 'C', 0.5)]);
        expect(result.ok).toBe(true);
        if (result.ok) {
