...
export function explainConfidence(score) {
    ...
    const levelWord = score.interpretation === 'HIGH'
        ? 'high'
...
function pickSupport(score, usedDriver) {
    const components = [
        { key: 'recency', value: score.components.recency },
        { key: 'sourceTier', value: score.components.sourceTier },
        { key: 'volatility', value: score.components.volatility },
    ...
    if (usedDriver !== null && usedDriver === ORDER_TEXT[best.key])
        return null;
    return text;
}
const ORDER_TEXT = {
    recency: 'The largest factor is recency: review cadence has slipped.',
    sourceTier: 'The largest factor is source: this value relies on an internal estimate rather than an external statistic.',
    agreement: 'The largest factor is cross-source agreement: alternative sources report materially different values.',
