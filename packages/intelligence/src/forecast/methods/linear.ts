...
export function forecastLinear(
  ...
  const minRequired =
    config.minMeasurementsRequired ?? DEFAULT_FORECAST_CONFIG.minMeasurementsRequired;
  ...
  if (measurements.length < minRequired) {
    return degraded(
      measurements[measurements.length - 1]!.observedValue,
      baseline,
      now,
      horizonDays,
    );
  ...
  const projectedAt = (days: number): number => intercept + slope * (daysToNow + days);
  ...
  const dayStdError =
    slope !== 0 ? Math.max(MIN_BREACH_DAYSE, Math.abs(slopeSE / slope) * 30) : Infinity;
  return {
    method: 'LINEAR',
