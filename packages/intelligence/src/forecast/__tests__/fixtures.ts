...
export function baseline(value = 100, tolerancePct = 10, unit = '% YoY'): AssumptionBaseline {
...
export function constantSeries(n: number, value: number, assumptionId = 'A-CONST'): Measurement[] {
  const measurements: Measurement[] = [];
  for (let i = 0; i < n; i += 1) {
    measurements.push({
      assumptionId,
