 ...
 */
export function computeLinearBreach(
  intercept: number,
  slope: number,
  ...
  if (slope !== 0) {
    ...
    if (Number.isFinite(tUpper) && tUpper >= 0) candidates.push(tUpper);
    if (Number.isFinite(tLower) && tLower >= 0) candidates.push(tLower);
  }
  if (intercept >= upper || intercept <= lower) candidates.push(0);
  if (candidates.length === 0) return null;
  const t = Math.min(...candidates);
  if (t > options.horizonDays) return null;
 ...
 */
export function computeIterativeBreach(
 ...
 */
export function breachCI(
  breachDate: Date | null,
  dayStdError: number,
  options: BreachOptions,
): [Date, Date] | null {
  if (breachDate === null || !Number.isFinite(dayStdError) || dayStdError < 0) return null;
