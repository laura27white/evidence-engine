...
const UNIT = '% YoY';
export async function fetchOnsCpi(options = {}) {
    ...
    for (const row of rows) {
        ...
        if (!parsedDate)
            continue;
        if (options.fromDate && parsedDate < options.fromDate)
            continue;
        if (options.toDate && parsedDate > options.toDate)
            continue;
 ...
 */
export function parseOnsValue(obs) {
