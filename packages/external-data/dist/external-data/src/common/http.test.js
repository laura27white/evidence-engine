...
describe('fetchJson', () => {
    it('returns ok on a 200 response', async () => {
        ...
        const result = await fetchJson('https://api.example.com/ping');
    ...
    it('surfaces a 404 as NOT_FOUND without retry', async () => {
        server.use(http.get('https://api.example.com/missing', () => HttpResponse.json({}, { status: 404 })));
    ...
    it('surfaces a non-retryable 4xx as HTTP', async () => {
        server.use(http.get('https://api.example.com/bad', () => HttpResponse.json({}, { status: 400 })));
    ...
    it('retries 5xx and eventually surfaces HTTP with retryable=true', async () => {
        let calls = 0;
        server.use(http.get('https://api.example.com/flaky', () => {
            calls += 1;
...
describe('fetchText', () => {
    it('returns the raw body for a 200 response', async () => {
        server.use(http.get('https://api.example.com/csv', () => HttpResponse.text('DATE,VALUE\n01 Jan 2025,5.25\n')));
