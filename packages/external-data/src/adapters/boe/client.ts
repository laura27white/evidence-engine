...
export function buildBoeUrl(path: BoeSeriesPath): string {
...
export async function fetchBoeCsv(path: BoeSeriesPath): Promise<
  FetchResult<{
...
function isoToIadbDate(input: string): string {
  if (input === 'now') {
    const today = new Date();
    return toDdMmmYyyy(today);
  }
  if (/^\d{2}\/[A-Za-z]{3}\/\d{4}$/.test(input)) return input;
  const parsed = new Date(input);
  if (Number.isNaN(parsed.getTime())) return input;
  return toDdMmmYyyy(parsed);
}

function toDdMmmYyyy(date: Date): string {
