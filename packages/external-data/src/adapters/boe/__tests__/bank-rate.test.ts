...
function mockBoeSuccess(): void {
  server.use(
    http.get('https://www.bankofengland.co.uk/boeapps/iadb/fromshowcolumns.asp', () =>
      HttpResponse.text(CSV_SAMPLE),
    ),
  );
}
...
describe('fetchBoeBankRate', () => {
  ...
  it('returns a SCHEMA error when the CSV has no data rows', async () => {
    server.use(
      http.get('https://www.bankofengland.co.uk/boeapps/iadb/fromshowcolumns.asp', () =>
        HttpResponse.text('DATE,IUDSOIA\n'),
      ),
    );
    const result = await fetchBoeBankRate();
