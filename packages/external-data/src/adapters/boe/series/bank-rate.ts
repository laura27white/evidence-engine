...
export async function fetchBoeBankRate(
  options: FetchBankRateOptions = {},
 ...
 */
export function parseIadbDate(raw: string): string | null {
...
export function parseIadbValue(raw: string): number | null {
  const trimmed = raw.trim();
  if (trimmed === '' || trimmed === '-99999' || trimmed === 'ND') return null;
