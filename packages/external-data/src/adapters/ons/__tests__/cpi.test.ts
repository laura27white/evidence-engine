...
function mockOnsSuccess(): void {
  server.use(
    http.get(buildOnsUrl({ seriesId: 'D7G7', dataset: 'mm23' }), () => HttpResponse.json(FIXTURE)),
  );
}
...
describe('fetchOnsCpi', () => {
  ...
  it('returns a SCHEMA error when the upstream payload is wrong shape', async () => {
    server.use(
      http.get(buildOnsUrl({ seriesId: 'D7G7', dataset: 'mm23' }), () =>
        HttpResponse.json({ months: 'not-an-array' }),
      ),
    );
    const result = await fetchOnsCpi();
